...
{Ramification polygon and polynomial phi(alpha+nu(alpha)x) of a nu-Oystein polynomial phi, where alpha is a root of phi. The slopes of the segments are in increasing order.}
...
The slopes of the segments are in increasing order.
}
          rp, rho := ramification_poly_raw(phi,alpha);
          LX<X> := Parent(rho);
          L := CoefficientRing(LX);
